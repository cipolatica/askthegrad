class User < ActiveRecord::Base
  ...
  def after_confirmation
    logger.debug "YAY! we're in after_confirmation"
    if (Registration.where(user_id:self.id).exists?)
      logger.debug "A Reg Exists!!!"
      reg = nil

      if Registration.where(user_id:self.id).exists?
        reg_list = Registration.where(user_id:self.id)
        reg = reg_list[reg_list.count - 1]
      end

      #reg = Registration.where(user_id:self.id).exists? ? Registration.where(user_id:self.id) : Registration.find(session[:reg_id]);
      #session[:reg_id] = nil
      ...
      authenticated_review.user_id = self.id
      authenticated_review.user_name = self.username

      the_current_user = self
      ...
      if (!should_navigate_to_root && authenticated_review.save)
        ...
        review_list = SchoolReview.where(school_id:authenticated_review.school_id).order(:major_id)
        ...
        reg.destroy
        self.update(reg:-1) #set reg = -1 so after_sign_in_path_for() method knows to show new review
        #session[:confirmed_user_updated] = "true"
        super
      else
        if not should_navigate_to_root
